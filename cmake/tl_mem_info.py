    ...
    def __init__(self, name, tool_nm):
        ...
        for l in proc.stdout:
            ...
            try:
                if fields[0] == b"U":
                    print(
                        "Warning: Undefined symbol '%s'!" % (fields[1].decode("ASCII"))
                    )
                    continue
                if fields[0] == b"w":
...
def main():
    ...
    sec_start = [
        ...
        b"_start_text_",
        b"_start_rodata_",
        b"_retention_data_start_",
        b"_retention_data_end_",
        b"_ictag_start_",
        b"_ictag_end_",
        b"_start_data_",
        b"_start_bss_",
    ...
    for i in range(len(sec_name)):
        ...
        print(
            "{0:>8}|{1:>21}|{2:>12X}|{3:>12X}|{4:>8d}".format(
                sec_name[i], sec_des[i], ss, se, sec_size[i]
            )
        )
    print("-------------------------------------------------------------------")
    ...
    print(
        "{0} : {1:d} {2}{3:X}{4}".format(
            "Start Load SRAM", load_sram, "(ICtag: 0x", ictag, ")"
        )
    )
    print(
        "{0} : {1:d} {2} {3}".format(
            "Total Used RRAM", retram_used, "from", chip_retram_size
        )
    )
    print(
        "{0} : {1:d} {2} {3}".format(
            "Total Used SRAM", ram_used, "from", chip_sram_size
        )
    )
    print(
        "{0} : {1:d}{2}{3}{4}{5}".format(
            "Total Free SRAM",
            sec_size[4],
            " + stack[",
            sec_size[10],
            "] = ",
            sec_size[4] + sec_size[10],
        )
    )
    if sec_size[10] < 256:
    ...
    if chip_retram_size < retram_used:
        print()
        print(
            "Error: Retention SRAM Overflow (%i bytes)!"
            % (retram_used - chip_retram_size),
            file=sys.stderr,
        )
        sys.exit(1)
    sys.exit(0)
